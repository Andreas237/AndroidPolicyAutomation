...
static final class FontsContractCompat$4
	implements Runnable
{
	...
	public void run()
	{
		...
		ntFamilyResult ntfamilyresult;
		ntInfo antinfo[];
		try
		{
			ntfamilyresult = FontsContractCompat.fetchFonts(val$context, ((android.os.CancellationSignal) (null)), val$request);
	//    0    0:aload_0         
	...
	//* 137  273:return          
		...
		if(ntfamilyresult.getStatusCode() != 0)
			switch(ntfamilyresult.getStatusCode())
			{
			default:
				val$callerThreadHandler.post(new Runnable() {
		...
		antinfo = ntfamilyresult.getFonts();
		if(antinfo != null && antinfo.length != 0)
		{
			int i = antinfo.length;
			for(final int resultCode = 0; resultCode < i; resultCode++)
			{
				ntInfo ntinfo = antinfo[resultCode];
				if(ntinfo.getResultCode() != 0)
				{
					resultCode = ntinfo.getResultCode();
					if(resultCode < 0)
					{
						val$callerThreadHandler.post(new Runnable() {
			...
			final Typeface typeface = FontsContractCompat.buildTypeface(val$context, ((android.os.CancellationSignal) (null)), antinfo);
			if(typeface == null)
			{
				val$callerThreadHandler.post(new Runnable() {
	...
	final ntRequestCallback val$callback;
	final Handler val$callerThreadHandler;
	final Context val$context;
	final FontRequest val$request;

	FontsContractCompat$4(Context context1, FontRequest fontrequest, Handler handler, ntRequestCallback ntrequestcallback)
	{
	...
	//    8   12:putfield        #45  <Field Handler val$callerThreadHandler>
		val$callback = ntrequestcallback;
	//    9   15:aload_0         
	//   10   16:aload           4
