// Decompiled by Jad v1.5.8g. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) annotate safe 

package com.google.android.gms.internal.ads;


// Referenced classes of package com.google.android.gms.internal.ads:
//			zzbcw, zzaxc, zzaxd, zzbah, 
//			zzaxg

public static final class zzaxc$zza extends zzbbo$zza
	implements zzbcw
{

	public final zzaxc$zza zzaf(zzbah zzbah)
	{
		((zzbbo$zza)this).zzadh();
	//    0    0:aload_0         
	//    1    1:invokevirtual   #28  <Method void zzbbo$zza.zzadh()>
		zzaxc.zza((zzaxc)zzdtx, zzbah);
	//    2    4:aload_0         
	//    3    5:getfield        #32  <Field zzbbo zzdtx>
	//    4    8:checkcast       #9   <Class zzaxc>
	//    5   11:aload_1         
	//    6   12:invokestatic    #35  <Method void zzaxc.zza(zzaxc, zzbah)>
		return this;
	//    7   15:aload_0         
	//    8   16:areturn         
	}

	public final zzaxc$zza zzav(int i)
	{
		((zzbbo$zza)this).zzadh();
	//    0    0:aload_0         
	//    1    1:invokevirtual   #28  <Method void zzbbo$zza.zzadh()>
		zzaxc.zza((zzaxc)zzdtx, 0);
	//    2    4:aload_0         
	//    3    5:getfield        #32  <Field zzbbo zzdtx>
	//    4    8:checkcast       #9   <Class zzaxc>
	//    5   11:iconst_0        
	//    6   12:invokestatic    #40  <Method void zzaxc.zza(zzaxc, int)>
		return this;
	//    7   15:aload_0         
	//    8   16:areturn         
	}

	public final zzaxc$zza zzc(zzaxg zzaxg)
	{
		((zzbbo$zza)this).zzadh();
	//    0    0:aload_0         
	//    1    1:invokevirtual   #28  <Method void zzbbo$zza.zzadh()>
		zzaxc.zza((zzaxc)zzdtx, zzaxg);
	//    2    4:aload_0         
	//    3    5:getfield        #32  <Field zzbbo zzdtx>
	//    4    8:checkcast       #9   <Class zzaxc>
	//    5   11:aload_1         
	//    6   12:invokestatic    #45  <Method void zzaxc.zza(zzaxc, zzaxg)>
		return this;
	//    7   15:aload_0         
	//    8   16:areturn         
	}

	private zzaxc$zza()
	{
		super(((zzbbo) (zzaxc.zzyp())));
	//    0    0:aload_0         
	//    1    1:invokestatic    #16  <Method zzaxc zzaxc.zzyp()>
	//    2    4:invokespecial   #19  <Method void zzbbo$zza(zzbbo)>
	//    3    7:return          
	}

	zzaxc$zza(zzaxd zzaxd)
	{
		this();
	//    0    0:aload_0         
	//    1    1:invokespecial   #23  <Method void zzaxc$zza()>
	//    2    4:return          
	}
}
