// Decompiled by Jad v1.5.8g. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) annotate safe 

package android.support.transition;

import android.graphics.Matrix;
import android.graphics.RectF;

class MatrixUtils
{

	private MatrixUtils()
	{
	//    0    0:aload_0         
	//    1    1:invokespecial   #17  <Method void Object()>
	//    2    4:return          
	}

	static final Matrix IDENTITY_MATRIX = new Matrix() {

		void oops()
		{
			throw new IllegalStateException("Matrix can not be modified");
		//    0    0:new             #14  <Class IllegalStateException>
		//    1    3:dup             
		//    2    4:ldc1            #16  <String "Matrix can not be modified">
		//    3    6:invokespecial   #19  <Method void IllegalStateException(String)>
		//    4    9:athrow          
		}

		public boolean postConcat(Matrix matrix)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean postRotate(float f)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean postRotate(float f, float f1, float f2)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean postScale(float f, float f1)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean postScale(float f, float f1, float f2, float f3)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean postSkew(float f, float f1)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean postSkew(float f, float f1, float f2, float f3)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean postTranslate(float f, float f1)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean preConcat(Matrix matrix)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean preRotate(float f)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean preRotate(float f, float f1, float f2)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean preScale(float f, float f1)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean preScale(float f, float f1, float f2, float f3)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean preSkew(float f, float f1)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean preSkew(float f, float f1, float f2, float f3)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean preTranslate(float f, float f1)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public void reset()
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

		public void set(Matrix matrix)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

		public boolean setConcat(Matrix matrix, Matrix matrix1)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean setPolyToPoly(float af[], int i, float af1[], int j, int k)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public boolean setRectToRect(RectF rectf, RectF rectf1, android.graphics.Matrix.ScaleToFit scaletofit)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
			return false;
		//    2    4:iconst_0        
		//    3    5:ireturn         
		}

		public void setRotate(float f)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

		public void setRotate(float f, float f1, float f2)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

		public void setScale(float f, float f1)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

		public void setScale(float f, float f1, float f2, float f3)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

		public void setSinCos(float f, float f1)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

		public void setSinCos(float f, float f1, float f2, float f3)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

		public void setSkew(float f, float f1)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

		public void setSkew(float f, float f1, float f2, float f3)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

		public void setTranslate(float f, float f1)
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

		public void setValues(float af[])
		{
			oops();
		//    0    0:aload_0         
		//    1    1:invokevirtual   #23  <Method void oops()>
		//    2    4:return          
		}

	}
;

	static 
	{
	//    0    0:new             #6   <Class MatrixUtils$1>
	//    1    3:dup             
	//    2    4:invokespecial   #13  <Method void MatrixUtils$1()>
	//    3    7:putstatic       #15  <Field Matrix IDENTITY_MATRIX>
	//*   4   10:return          
	}
}
