// Decompiled by Jad v1.5.8g. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) annotate safe 

package com.facebook.shimmer;


// Referenced classes of package com.facebook.shimmer:
//			ShimmerFrameLayout

public static final class ShimmerFrameLayout$MaskShape extends Enum
{

	public static ShimmerFrameLayout$MaskShape valueOf(String s)
	{
		return (ShimmerFrameLayout$MaskShape)Enum.valueOf(com/facebook/shimmer/ShimmerFrameLayout$MaskShape, s);
	//    0    0:ldc1            #2   <Class ShimmerFrameLayout$MaskShape>
	//    1    2:aload_0         
	//    2    3:invokestatic    #35  <Method Enum Enum.valueOf(Class, String)>
	//    3    6:checkcast       #2   <Class ShimmerFrameLayout$MaskShape>
	//    4    9:areturn         
	}

	public static ShimmerFrameLayout$MaskShape[] values()
	{
		return (ShimmerFrameLayout$MaskShape[])((ShimmerFrameLayout$MaskShape []) ($VALUES)).clone();
	//    0    0:getstatic       #27  <Field ShimmerFrameLayout$MaskShape[] $VALUES>
	//    1    3:invokevirtual   #42  <Method Object _5B_Lcom.facebook.shimmer.ShimmerFrameLayout$MaskShape_3B_.clone()>
	//    2    6:checkcast       #38  <Class ShimmerFrameLayout$MaskShape[]>
	//    3    9:areturn         
	}

	private static final ShimmerFrameLayout$MaskShape $VALUES[];
	public static final ShimmerFrameLayout$MaskShape LINEAR;
	public static final ShimmerFrameLayout$MaskShape RADIAL;

	static 
	{
		LINEAR = new ShimmerFrameLayout$MaskShape("LINEAR", 0);
	//    0    0:new             #2   <Class ShimmerFrameLayout$MaskShape>
	//    1    3:dup             
	//    2    4:ldc1            #16  <String "LINEAR">
	//    3    6:iconst_0        
	//    4    7:invokespecial   #20  <Method void ShimmerFrameLayout$MaskShape(String, int)>
	//    5   10:putstatic       #22  <Field ShimmerFrameLayout$MaskShape LINEAR>
		RADIAL = new ShimmerFrameLayout$MaskShape("RADIAL", 1);
	//    6   13:new             #2   <Class ShimmerFrameLayout$MaskShape>
	//    7   16:dup             
	//    8   17:ldc1            #23  <String "RADIAL">
	//    9   19:iconst_1        
	//   10   20:invokespecial   #20  <Method void ShimmerFrameLayout$MaskShape(String, int)>
	//   11   23:putstatic       #25  <Field ShimmerFrameLayout$MaskShape RADIAL>
		$VALUES = (new ShimmerFrameLayout$MaskShape[] {
			LINEAR, RADIAL
		});
	//   12   26:iconst_2        
	//   13   27:anewarray       ShimmerFrameLayout$MaskShape[]
	//   14   30:dup             
	//   15   31:iconst_0        
	//   16   32:getstatic       #22  <Field ShimmerFrameLayout$MaskShape LINEAR>
	//   17   35:aastore         
	//   18   36:dup             
	//   19   37:iconst_1        
	//   20   38:getstatic       #25  <Field ShimmerFrameLayout$MaskShape RADIAL>
	//   21   41:aastore         
	//   22   42:putstatic       #27  <Field ShimmerFrameLayout$MaskShape[] $VALUES>
	//*  23   45:return          
	}

	private ShimmerFrameLayout$MaskShape(String s, int i)
	{
		super(s, i);
	//    0    0:aload_0         
	//    1    1:aload_1         
	//    2    2:iload_2         
	//    3    3:invokespecial   #29  <Method void Enum(String, int)>
	//    4    6:return          
	}
}
