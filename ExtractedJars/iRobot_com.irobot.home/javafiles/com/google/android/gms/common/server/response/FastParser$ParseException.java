// Decompiled by Jad v1.5.8g. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) annotate safe 

package com.google.android.gms.common.server.response;


// Referenced classes of package com.google.android.gms.common.server.response:
//			FastParser

public static class FastParser$ParseException extends Exception
{

	public FastParser$ParseException(String s)
	{
		super(s);
	//    0    0:aload_0         
	//    1    1:aload_1         
	//    2    2:invokespecial   #13  <Method void Exception(String)>
	//    3    5:return          
	}

	public FastParser$ParseException(String s, Throwable throwable)
	{
		super(s, throwable);
	//    0    0:aload_0         
	//    1    1:aload_1         
	//    2    2:aload_2         
	//    3    3:invokespecial   #17  <Method void Exception(String, Throwable)>
	//    4    6:return          
	}

	public FastParser$ParseException(Throwable throwable)
	{
		super(throwable);
	//    0    0:aload_0         
	//    1    1:aload_1         
	//    2    2:invokespecial   #20  <Method void Exception(Throwable)>
	//    3    5:return          
	}
}
